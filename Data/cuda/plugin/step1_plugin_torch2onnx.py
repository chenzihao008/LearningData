...
# 这种方式可以直接输出onnx
# 但是onnx应该是不可执行的，因为onnx中没有该"custom::customScalar"算子
# 但是不重要，因为要转成tensorrt
## 创建onnx映射
class CustomScalarImpl(torch.autograd.Function):
    @staticmethod
    def symbolic(g, x, r, s):
        return g.op("custom::customScalar", x, scalar_f=r, scale_f=s) #scalar_f 中的 _f 是必须写的，表示浮点数，如果是整型的话，加_i,sting 加 _s ；"custom::customScalar"分别是c++的namespace和算子名字
...
class CustomScalar(nn.Module):
    ...
    def forward(self, x):
        return CustomScalarImpl.apply(x, self.scalar, self.scale) # 传入初始化好的self.scalar, self.scale参数
