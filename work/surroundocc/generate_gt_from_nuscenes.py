...
def run_poisson(pcd, depth, n_threads, min_density=None):
    ...
    if min_density:
        vertices_to_remove = densities < np.quantile(densities, min_density)#去掉低密度的部分顶点
...
def main(nusc, val_list, indice, nuscenesyaml, args, config):
    ...
    dict_list = []
    #### 逐帧获取数据（seg、bbox等动态物体、静态场景、lidar标定位姿参数）
    while True:
        ############################# get boxes ##########################
        ...
        gt_bbox_3d[:, 2] -= dims[:, 2] / 2.   # 使center的z移动到bbox的最下端
        ...
        ### 获取静态的点，semantics LiDAR points 去掉bbox内的点剩下就是静态的
        ...
        lidar_pc = lidar_to_world_to_lidar(pc.copy(), lidar_calibrated_sensor.copy(), lidar_ego_pose.copy(),
                                           ...
                                           lidar_ego_pose0) #静态point转到第一帧的lidar视角下
        ################## record Non-key frame information into a dict  ########################
        dict = {"object_tokens": object_tokens,#所有有标注实例的token上
                "object_points_list": object_points_list,#objectpoint 没有转到第一帧视角下即在全局视角下
                "lidar_pc": lidar_pc.points,#静态点，转到第一帧lidar视角下
        ...
        if lidar_data['is_key_frame']:
            ...
            lidar_pc_with_semantic = lidar_to_world_to_lidar(pc_with_semantic.copy(),
                                                             ...
                                                             lidar_ego_pose.copy(),
                                                             lidar_calibrated_sensor0,
                                                             lidar_ego_pose0) # 带分割数据的点转到第一帧的lidar视角下
    ...
    object_token_zoo = []#所有帧的object
    object_semantic = []#所有帧的类别
    for dict in dict_list:
    ...
    for query_object_token in object_token_zoo:
            ...
            for i, object_token in enumerate(dict['object_tokens']):
                if query_object_token == object_token:
                    ...
                    if object_points.shape[0] > 0:
                        object_points = object_points[:,:3] - dict['gt_bbox_3d'][i][:3] #相对bbox center的位置
                        rots = dict['gt_bbox_3d'][i][6]
                        Rot = Rotation.from_euler('z', -rots, degrees=False)#转成没有航向角度的
        ...
        object_points_dict[query_object_token] = np.concatenate(object_points_dict[query_object_token],
                                                                axis=0)#所有帧下同一个object相对bbox的point 合并到一起
    ...
    while int(i) < 10000:  # Assuming the sequence does not have more than 10000 frames
        ...
        ################## bbox placement ##############
        object_points_list = []#该帧下，该lidar视角下的所有object包含分别包含的point
        ...
        for j, object_token in enumerate(dict['object_tokens']):
            for k, object_token_in_zoo in enumerate(object_token_zoo):
                if object_token==object_token_in_zoo:#从上面无航向角的的objectdict中获取该object的point
                    points = object_points_vertice[k]
                    Rot = Rotation.from_euler('z', rots[j], degrees=False)
                    rotated_object_points = Rot.apply(points)
                    points = rotated_object_points + locs[j]#point转换成该视角该bbox的rot下
                    if points.shape[0] >= 5: 
                    ...
                    object_semantic_list.append(np.concatenate([points[:, :3], semantics], axis=1))#比object_points_list 多了个semantic而已

        try: # avoid concatenate an empty array
            temp = np.concatenate(object_points_list)
            scene_points = np.concatenate([point_cloud, temp])#跟静态点合并
        except:
        ...
        scene_points = scene_points[mask]#获取在前后左右50m且上下8m内的点
        ...
        scene_points = np.asarray(mesh.vertices, dtype=float)##获取顶点
        ...
        fov_voxels = vv[gt_ > 0]#过滤出有占用的voxel
        fov_voxels[:, :3] = (fov_voxels[:, :3] + 0.5) * voxel_size#这里似乎有问题，完全没有赋值
        ...
        dense_voxels_with_semantic = np.concatenate([fov_voxels, dense_semantic[:, np.newaxis]], axis=1)#voxel chamfer 搜索 semantics
